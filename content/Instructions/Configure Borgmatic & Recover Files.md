...
Borgmatic uses .yaml config files to specify a backup job. 
The config file can be read and understood using this system:

1. What do I want backed up (`source_directories`)
2. Where do I want the backup stored (`repositories`)
3. How long do I want to keep my backups? (`retention`)
4. When do I want to check my backups? (`consistency`)
5. Do I want any extra steps done (pause / unpause Docker containers, scripts run, etc?) (`hooks`)
6. Which, if any, databases do I want backup up? (`databases`)
8. Which, if any, notification services do I want to use? (`ntfy and below`)
9. Which notifications do I want to receive? (`states`)
...
`Apprise` is enabled (just uncomment a section and it's enabled) and commands for both Gmail and Discord are already typed in. Please refer to [Apprise wiki](https://github.com/caronc/apprise/wiki) for detailed instructions on how to configure different notification services. 
